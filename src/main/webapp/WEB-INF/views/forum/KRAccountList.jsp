				...
				<c:forEach items="${ acclist }" var="acclist"
					...
					<div class="table-row large">
						...
						<div class="table-row-item left">
							<canvas id="chart_${acclist.accountid}" height=70 width=125></canvas>
						</div>
						<div class="table-row-item left">
...
function getprofitchartdata(id) {
	$
			.ajax({
				url : '${pageContext.request.contextPath}/forum/getKRprofitchartdata?id='+id,
				type : 'get',
				async : false,
				data : '',
				success : function(data) {
					console.log(data);
					 generatechart(id,JSON.parse(data.result));
				},
...
getprofitchartdata('${acclist1.accountid}');
</c:forEach>
