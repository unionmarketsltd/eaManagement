...
var searchdata;
	...
	function fetchgroup()
	{ var url = '${pageContext.request.contextPath}/fundmanager/getgroup';
	 ...
	 const select = document.getElementById('managergroupselect');
	  ...
	  {
		  showloading(1);
		  var urla = '${pageContext.request.contextPath}/fundmanager/CustomerServicesGetCommentUser';
		 ...
		 $ .ajax({
				...
				success : function(data) {
					    ...
					    if(jobj.retcode.indexOf("0 Done")>=0)
					    	{
					    	const obj = JSON.parse(jobj.answer[0].Text).All;
		...
		tableclick.addEventListener("click", function(event) {
		  if (event.target.nodeName === "TD") {
		   ...
		   // alert("You clicked on row: " + row.children[0].innerHTML);
		    redirecttotopic(row.children[2].innerHTML,row.children[1].innerHTML);
		...
		{ var url = '${pageContext.request.contextPath}/fundmanager/viewCustomerTopic?tab=new&id='+id+'&fm='+fm;
