...
var searchdata;
	function convertUnixtoLocalTime(unixTimestamp) {
	  ...
	  {
		  showloading(1);
		  var urla = '${pageContext.request.contextPath}/fundmanager/getFundHistory';
					 ...
					 console.log(data);
					    const jobj = JSON.parse(data.result);
					    	  ...
					    	  if(globaldata.length>0)
						    	...
						    	{
						    	alert("No Fund History Record Found.");
	...
	{ if(pf.indexOf('-')>=0)
		...
		function insertTableRowWithSelectedColumn(tableid, array,
			...
			for (let i = array.length - 1; i >= 0; i--) {
				...
				for (let j = 0; j < fieldIndex.length; j++) {
					...
					else if (header[fieldIndex[keylength - j]] === "Comment") //datetimeconvert
					{
						cell.innerHTML = codetostring(Object
								.values(array[i])[fieldIndex[keylength - j]]
								.toString());
					} 
		...
		function codetostring(code) {
			...
			if (code.indexOf("M2P")<0)
				{
				var extractedNumbers = code.match(/\d+/g);
	            if (extractedNumbers != null && extractedNumbers.length == 3) {
	                var output = extractedNumbers[0] + extractAction(code) + " FROM " + extractedNumbers[1] + " TO " + extractedNumbers[2]
	                return output;
        ...
        function extractAction(str) {
            const alphabet = str.match(/[A-Z]/g);
            if (alphabet[0] === 'D') {
                return " PERFORM DEPOSIT";
            } else if (alphabet[0] === 'W') {
                return " PERFORM WITHDRAWAL";
            } else if (alphabet[0] === 'T') {
                return " PERFORM TRANSFER";
            } else {
                return " PERFORM UNKNOWN ACTION";
            }
        }
