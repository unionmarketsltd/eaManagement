...
function prevpage()
	...
	function getMasterFromPlugin() {
		$
				.ajax({
					url : '${pageContext.request.contextPath}/fundclient/getMasterFromPlugin',
					type : 'get',
					async : false,
					data : $("#accountListFrm").serialize(),
					success : function(data) {
						if (data.result.indexOf("0 Done" >= 0)) {
							...
							if (jobject.retcode == "0 Done") {
								if (jobject.hasOwnProperty('answer')) {
									...
									insertTableRowWithAllColumn("datatable", Jarrayanswer);
									}
								else
									{
	...
	function insertTableRowWithSelectedColumn(tableid, array,
		...
		const fieldtoshow = [ "Order", "Symbol", "Type", "Volume",
