...
function search()
{
	...
	function convertUnixtoLocalTime(unixTimestamp) {
	  ...
	  {
		  showloading(1);
		  var urla = '${pageContext.request.contextPath}/fundmanager/getlth?id='+inputlogin;
				...
				success : function(data) {
					 console.log(data);
					    ...
					    if(jobj.answer.length>0)
					    	...
					    	alert("This account do not have any trading history.")
					    	}
	...
	{ if(pf.indexOf('-')>=0)
		...
		function insertTableRowWithSelectedColumn(tableid, array,
				enableButtonAction) {
			const fieldtoshow = [ "Deal","PricePosition", "Symbol", "Action",
					"Registration" ,"Time","Profit","Price","Storage"];
			var fieldIndex = [];
			...
			for (let i = 0; i < header.length; i++) {
				...
				for (let j = 0; j < fieldtoshow.length; j++) {
					if (header[i] === fieldtoshow[j]) {
						...
						else if(header[i].indexOf("Time")>=0)
						{
						th.innerHTML = "Open Time";
						}
						
						
						else if(header[i].indexOf("PricePosition")>=0)
						{
						th.innerHTML = "Price Closed";
						}
						
						else if(header[i].localeCompare("Price") ==0)
						{
			...
			$(function() {
				jQuery.each($("table tr"), function() {
					...
					$(this).children(":eq(7)").after($(this).children(":eq(5)")); // rearange col 2 to 1
					$(this).children(":eq(6)").after($(this).children(":eq(5)")); // rearange col 2 to 1
				});
			});
		}
