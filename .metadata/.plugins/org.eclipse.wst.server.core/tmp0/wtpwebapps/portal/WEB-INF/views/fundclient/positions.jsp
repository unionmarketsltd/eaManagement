...
<script type="text/javascript">
	...
	function GetTotalPosition() {
		$.ajax({
			url : '${pageContext.request.contextPath}/fundclient/PositionGetTotal',
			type : 'get',
			async : false,
	...
	function GetPositionList(page) {
		$
				.ajax({
					url : '${pageContext.request.contextPath}/fundclient/PositionGetList?all=1&page='
							+ page,
					type : 'get',
					async : false,
					data : $("#accountListFrm").serialize(),
	...
	function GetPositionSummary100() {
		$
				.ajax({
					url : '${pageContext.request.contextPath}/fundclient/PositionGet100',
					type : 'get',
					async : false,
					data : "",
					success : function(data) {
	...
	function insertTableRowWithSelectedColumn(tableid, array,
		...
		for (let i = array.length - 1; i >= 0; i--) {
			...
			if (enableButtonAction) {
				...
				cell.innerHTML = "<button  onclick='openmodalconfirm(" + positionid + ",\""+symbol+"\","+lot+",\""+type+"\")' class='btn btn-default' style='display:none'>Close</button>";
			}
	...
	function closedPosition(position,symbol,lot,type)
	{
		...
		$
		.ajax({
			url : "${pageContext.request.contextPath}/fundclient/closePosition?pid="+position+"&symbol="+symbol+"&lot="+lot+"&type="+type,
			type : 'get',
			async : false,
			data : "",
			success : function(data) {
