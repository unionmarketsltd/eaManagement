...
<script type="text/javascript">
	...
	function check() {
		$
				.ajax({
					url : '${pageContext.request.contextPath}/crm/getAccountList',
					type : 'get',
					async : false,
						...
						if (data.result.indexOf("0 Done" >= 0)) {
							...
							if (jobject.retcode == "0 Done") {
								if (jobject.hasOwnProperty('answer')) {
									jarray = jobject.answer;
									document.getElementById("jsonoutput").innerHTML = jarray.length;
									//insertTableRow("datatable", jarray);
									insertTableRowWithSelectedColumn(
											"datatable", jarray, true);
								} else {
	...
	function insertTableRowWithSelectedColumn(tableid, array,
			enableButtonAction) {
		const fieldtoshow = [ "Login", "Name", "Leverage", "Balance",
				"Registration" ];
		var fieldIndex = [];
