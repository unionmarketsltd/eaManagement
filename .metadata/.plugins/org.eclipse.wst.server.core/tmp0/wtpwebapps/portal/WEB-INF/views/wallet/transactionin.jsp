						...
						<div class="col-12">
							<div class="card" id="tablediv" >
								<div class="card-header">
									<h3 class="card-title" style="line-height:2">History&nbsp;&nbsp;&nbsp;</h3>
									<select class="custom-select rounded-0" id="mySelect" style="width:20% !important"></select>
									<h3 id="pagenumber" style="line-height:2" class="float-right card-title"></h3>
...
function search()
{
	...
	function fetchcointype()
	  {
		  showloading(1);
		  var urla = '${pageContext.request.contextPath}/wallet/getcointype';
				...
				success : function(data) {
					 console.log(data);
					    ...
					    if(jobj.retcode.indexOf("0 Done")>=0)
					    	{
					    	if( jobj.answer.data != null && jobj.answer.data != 'null')
					    		{
					    		...
					    		for( var i=0; i<jobj.answer.data.length;i++)
					    		{
					    		console.log(jobj.answer.data[i].coin_type);
					    		const option = document.createElement("option");
					    		  option.text = jobj.answer.data[i].coin_type;
					    		  option.value = jobj.answer.data[i].coin_type;
					    		  mySelect.add(option);
	  ...
	  const coinselect = document.getElementById("mySelect");
	  ...
	  function fetchtransactions(cointype)
	  {
		  showloading(1);
		  var urla = '${pageContext.request.contextPath}/wallet/getTransactionList?coin_type='+cointype;
	...
	{ if(pf.indexOf('-')>=0)
		...
		function insertTableRowWithSelectedColumn(tableid, array,
				enableButtonAction) {
			const fieldtoshow = [ "Action", "Coin Amount", "Coin Type",
			...
			for (let i = 0; i < header.length; i++) {
				...
				for (let j = 0; j < fieldtoshow.length; j++) {
					if (header[i] === fieldtoshow[j]) {
						...
						if(header[i].indexOf("Storage")>=0)
							{
							th.innerHTML = "Swap";
							}
						else if(header[i].indexOf("Deal")>=0)
						{
						th.innerHTML = "Ticket";
						}
						else
			...
			for (let i = array.length - 1; i >= 0; i--) {
				...
				for (let j = 0; j < fieldIndex.length; j++) {
					...
					if (header[fieldIndex[keylength - j]] === "Time") //datetimeconvert
					{
					...
					{
						cell.innerHTML = convertaction(Object
								.values(array[i])[fieldIndex[keylength - j]]
								.toString());
					} 
					
					else if (header[fieldIndex[keylength - j]] === "In/Out") //datetimeconvert
					{
						cell.innerHTML = convertinout(Object
								.values(array[i])[fieldIndex[keylength - j]]
								.toString());
					} 
