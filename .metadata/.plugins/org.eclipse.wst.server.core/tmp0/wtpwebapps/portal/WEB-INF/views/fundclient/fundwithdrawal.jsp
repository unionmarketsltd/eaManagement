						...
						<div class="col-12">
							<div class="card">
								<div class="card-header">
									<h3 class="card-title"><a href="newWithdrawalRequest">Create New Withdrawal Request+</a></h3>
...
var currentPage = 0;
	...
	function convertUnixtoLocalTime(unixTimestamp) {
	 ...
	 function InitialFundConfugurationComment() {
			$
					.ajax({
						url : "${pageContext.request.contextPath}/fundclient/InitialFundConfugurationComment",
						type : 'get',
	  ...
	  function fetchgrouppositions( )
	  {
		  ...
		  var urla = '${pageContext.request.contextPath}/fundclient/GetFundConfugurationComment';
		 ...
		 $ .ajax({
				...
				success : function(data) {
					   ...
					   if(answer.CommentConfiguration.length >0)
						   ...
						   console.log(answer.CommentConfiguration.length);
						   
						   answer.CommentConfiguration.sort(function(a, b) {
		...
		{ var url = '${pageContext.request.contextPath}/fundclient/getAccountDetails?id='+id;
	...
	function convertwhoreplied(bs)
	{
		if( bs == "0" )
			{
			return '<span class="right badge badge-info">Customer Replied</span>';
			}
		else
			{
			return '<span class="right badge badge-danger">Agent Replied</span>';
			}
	}
		...
		function insertTableRowWithSelectedColumn(tableid, array,
			...
			for (let i = array.length - 1; i >= 0; i--) {
				...
				if (enableButtonAction) {
					...
					cell.innerHTML = "<a href='viewWithdrawalRequest?"+"status="+status+"&index=" + index+"&title="+title
