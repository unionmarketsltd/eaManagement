...
if ($('#pageWrapper').hasClass('compact-wrapper')) {
    $(".sidebar-wrapper nav").find("a").removeClass("active");
    $(".sidebar-wrapper nav").find("li").removeClass("active");
	// 2021-08-11 menu bar link add
    var current = window.location.pathname
    var currentparameter = window.location.search;
    
    if (currentparameter.indexOf('paytype=all') != -1 ) current = current + '?paytype=all';
    else if (currentparameter.indexOf('paytype=Deposit') != -1 ) current = current + '?paytype=Deposit';  
    else if (currentparameter.indexOf('paytype=Withdrawal') != -1 ) current = current + '?paytype=Withdrawal';  
    else if (currentparameter.indexOf('paytype=Transfer') != -1 ) current = current + '?paytype=Transfer';
    else if (current.indexOf('userprofile') != -1 ) current = 'userlist';
    
    //Basic
    else if (currentparameter.indexOf('planType=Basic') != -1 ) current = current + '?planType=Basic';
    else if (currentparameter.indexOf('planType=Dadream') != -1 ) current = current + '?planType=Dadream';
    
    if (current.indexOf('bankWireDeposit') != -1 ) current = '/portal/trader/deposit';
      
    $(".sidebar-wrapper nav ul>li a").filter(function () {
        var link = $(this).attr("href");
        console.log('###### link222 : ' + link + ':' + current + ':' + currentparameter);
        if (link) {
            if (current.indexOf(link) != -1) {
                $(this).parents().children('a').addClass('active');
